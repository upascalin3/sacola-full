...
import { LivestockEntryData } from "@/lib/socio-economic/socio-economic";
import SearchAndFilters from "./components/SearchAndFilters";
import Pagination from "./components/Pagination";

const initialLivestockData: LivestockEntryData[] = [  
  {
    id: "1",
    animalType: "Goats",
    location: "Nyange",
...
export default function SocioEconomicPage() {
  ...
  const [treeData, setTreeData] = useState<LivestockEntryData[]>(
  ...
  const [selectedEntry, setSelectedEntry] = useState<LivestockEntryData | null>(
    null
  );
  ...
  const handleCreateEntry = (data: SocioEconomicData) => {
    const newEntry: LivestockEntryData = {
      ...(data as Omit<LivestockEntryData, "id">),
  ...
  const handleViewDetails = (entry: LivestockEntryData) => {
    router.push(`/dashboard/socio-economic?id=${entry.id}`);
  };

  const handleCloseDetails = () => {
    setIsDetailsModalOpen(false);
    setSelectedEntry(null);
    router.push("/dashboard/socio-economic");
  };
  ...
  const handleUpdateEntry = (data: SocioEconomicData) => {
    if (selectedEntry) {
      const updatedEntry: LivestockEntryData = {
        ...(data as Omit<LivestockEntryData, "id">),
        id: selectedEntry.id,
      };
      setTreeData((prev) =>
  ...
  return (
    <div className="ml-64">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedData.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {item.distributedAnimals.toLocaleString()}
                        </td>
      ...
      <DeleteConfirmationModal
        ...
        socioEconomicType="livestock"
        itemName={selectedEntry?.animalType}
      />
    </div>
  );
}
