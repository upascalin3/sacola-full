...
import { HousingMaterialsEntryData } from "@/lib/socio-economic/socio-economic";
import SearchAndFilters from "../../components/SearchAndFilters";
import Pagination from "../../components/Pagination";

const Data: HousingMaterialsEntryData[] = [  
  {
    id: "1",
...
export default function HousingMaterialsPage() {
  ...
  const [treeData, setTreeData] = useState<HousingMaterialsEntryData[]>(
    Data
  );
  ...
  const [selectedEntry, setSelectedEntry] = useState<HousingMaterialsEntryData | null>(
    null
  );
  ...
  const handleCreateEntry = (data: SocioEconomicData) => {
    const newEntry: HousingMaterialsEntryData = {
      ...(data as Omit<HousingMaterialsEntryData, "id">),
  ...
  const handleViewDetails = (entry: HousingMaterialsEntryData) => {
    router.push(`/dashboard/socio-economic/housing/materials?id=${entry.id}`);
  };

  const handleCloseDetails = () => {
    setIsDetailsModalOpen(false);
    setSelectedEntry(null);
    router.push("/dashboard/socio-economic/housing/materials");
  };
  ...
  const handleUpdateEntry = (data: SocioEconomicData) => {
    if (selectedEntry) {
      const updatedEntry: HousingMaterialsEntryData = {
        ...(data as Omit<HousingMaterialsEntryData, "id">),
        id: selectedEntry.id,
      };
      setTreeData((prev) =>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {paginatedData.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {item.currentBeneficiaries.toLocaleString()}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {item.distributedMaterials.toLocaleString()}
                        </td>
