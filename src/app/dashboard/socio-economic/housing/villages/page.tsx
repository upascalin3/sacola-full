...
import { HousingVillagesEntryData } from "@/lib/socio-economic/socio-economic";
import SearchAndFilters from "../../components/SearchAndFilters";
import Pagination from "../../components/Pagination";

const Data: HousingVillagesEntryData[] = [  
  {
    id: "1",
...
export default function housingVillagesPage() {
  ...
  const [treeData, setTreeData] = useState<HousingVillagesEntryData[]>(
    Data
  );
  ...
  const [selectedEntry, setSelectedEntry] = useState<HousingVillagesEntryData | null>(
    null
  );
  ...
  const handleCreateEntry = (data: SocioEconomicData) => {
    const newEntry: HousingVillagesEntryData = {
      ...(data as Omit<HousingVillagesEntryData, "id">),
  ...
  const handleViewDetails = (entry: HousingVillagesEntryData) => {
    router.push(`/dashboard/socio-economic/housing/villages?id=${entry.id}`);
  };

  const handleCloseDetails = () => {
    setIsDetailsModalOpen(false);
    setSelectedEntry(null);
    router.push("/dashboard/socio-economic/housing/villages");
  };
  ...
  const handleUpdateEntry = (data: SocioEconomicData) => {
    if (selectedEntry) {
      const updatedEntry: HousingVillagesEntryData = {
        ...(data as Omit<HousingVillagesEntryData, "id">),
        id: selectedEntry.id,
      };
      setTreeData((prev) =>
  ...
  return (
    <div className="ml-64">
      ...
      <DeleteConfirmationModal
        ...
        socioEconomicType="housingVillages"
        itemName={selectedEntry?.villageName}
      />
    </div>
  );
}
