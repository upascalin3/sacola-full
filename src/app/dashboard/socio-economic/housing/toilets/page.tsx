...
import { HousingToiletsEntryData } from "@/lib/socio-economic/socio-economic";
import SearchAndFilters from "../../components/SearchAndFilters";
import Pagination from "../../components/Pagination";

const Data: HousingToiletsEntryData[] = [  
  {
    id: "1",
    toiletType: "WC",
    toiletsBuilt: 69,
...
export default function housingToiletsPage() {
  ...
  const [treeData, setTreeData] = useState<HousingToiletsEntryData[]>(
    Data
  );
  ...
  const [selectedEntry, setSelectedEntry] = useState<HousingToiletsEntryData | null>(
    null
  );
  ...
  const filteredData = useMemo(() => {
    return treeData.filter((item) => {
      const matchesSearch =
        searchTerm === "" ||
        item.toiletType.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleCreateEntry = (data: SocioEconomicData) => {
    const newEntry: HousingToiletsEntryData = {
      ...(data as Omit<HousingToiletsEntryData, "id">),
  ...
  const handleViewDetails = (entry: HousingToiletsEntryData) => {
    router.push(`/dashboard/socio-economic/housing/toilets?id=${entry.id}`);
  };

  const handleCloseDetails = () => {
    setIsDetailsModalOpen(false);
    setSelectedEntry(null);
    router.push("/dashboard/socio-economic/housing/toilets");
  };
  ...
  const handleUpdateEntry = (data: SocioEconomicData) => {
    if (selectedEntry) {
      const updatedEntry: HousingToiletsEntryData = {
        ...(data as Omit<HousingToiletsEntryData, "id">),
        id: selectedEntry.id,
      };
      setTreeData((prev) =>
