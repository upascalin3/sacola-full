...
import { empowermentMicroFinanceEntryData } from "@/lib/socio-economic/socio-economic";
import SearchAndFilters from "../../components/SearchAndFilters";
import Pagination from "../../components/Pagination";

const Data: empowermentMicroFinanceEntryData[] = [  
  {
    id: "1",
    name: "Tuzamurane",
    location: "Nyange",
...
export default function empowermentMicroFinancePage() {
  ...
  const [treeData, setTreeData] = useState<empowermentMicroFinanceEntryData[]>(
    Data
  );
  ...
  const [selectedEntry, setSelectedEntry] = useState<empowermentMicroFinanceEntryData | null>(
    null
  );
  ...
  const handleCreateEntry = (data: SocioEconomicData) => {
    const newEntry: empowermentMicroFinanceEntryData = {
      ...(data as Omit<empowermentMicroFinanceEntryData, "id">),
  ...
  const handleViewDetails = (entry: empowermentMicroFinanceEntryData) => {
    router.push(`/dashboard/socio-economic/empowerment/micro-finance?id=${entry.id}`);
  };

  const handleCloseDetails = () => {
    setIsDetailsModalOpen(false);
    setSelectedEntry(null);
    router.push("/dashboard/socio-economic/empowerment/micro-finance");
  };
  ...
  const handleUpdateEntry = (data: SocioEconomicData) => {
    if (selectedEntry) {
      const updatedEntry: empowermentMicroFinanceEntryData = {
        ...(data as Omit<empowermentMicroFinanceEntryData, "id">),
        id: selectedEntry.id,
      };
      setTreeData((prev) =>
